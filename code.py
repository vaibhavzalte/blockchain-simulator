# todo 
# transaction pickup random in n => done
import random
...
class Blockchain:
    ...
    def add_transaction(self, sender, receiver, amount):
        ...
        self.transactions.append({'transaction_id': transaction_id,'sender': sender, 'receiver': receiver, 'amount': amount})
...
def get_highest_transactions():
    ...
    top_transactions = sorted(blockchain.transactions, key=itemgetter('amount'), reverse=True)[:k]
...
def remove_highest_transactions():
    ...
    # Remove the highest k transactions from all nodes
    for transaction in top_transactions:
...
def mine_block_with_transactions():
    ...
    response = requests.get(f'http://{mining_node}/remove_highest_transactions?k={n_high}')
    ...
    response = requests.post(f'http://{mining_node}/add_time',
                             json={'block_time': (end_time-start_time)/n_high})
...
def add_time():
    try:
        time = request.json['block_time']
        blockchain.throughput.append(time)
        return jsonify({'message': 'Time added successfully'}), 200
    except KeyError:
        return jsonify({'error': 'Time not provided in the request'}), 400
...
def average_throughput():
    ...
    if not throughput_list:
        return jsonify({'error': 'No throughput data available'}), 404
